...
describe("ERC20 Kronos", function () {
  it("Testing the contract", async function () {
    ...
    await krn.connect(deployer).mint(deployer.address, ethers.utils.parseUnits("250", "ether"));
    supply = await krn.totalSupply();


    console.log('-----------------------------------------------------------');
    console.log("Show second call!");
    expect(supply).to.equal(ethers.utils.parseUnits("250", "ether"));

    
    console.log('-----------------------------------------------------------');
    console.log("Third call");
    try {
        await krn.connect(user0).mint(user0.address, ethers.utils.parseUnits("250", "ether"));
    } catch(e) {
        console.log("Error");
    }
    supply = await krn.totalSupply();
